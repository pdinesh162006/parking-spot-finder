...
import { Car, Mail, Lock, User, Phone, Loader2, ArrowRight } from 'lucide-react';
...
export default function Register() {
  ...
  const handleGoogleSuccess = async (googleEmail: string, googleName: string) => {
    ...
    try {
      const res = await fetch(`${API_URL}/auth/google-login`, {
        ...
        body: JSON.stringify({ email: googleEmail, name: googleName, role }),
      });
  ...
  return (
    <div className="min-h-[85vh] flex items-center justify-center px-4 sm:px-6 lg:px-8 py-8">
      <div className="max-w-md w-full space-y-6 bg-white p-8 rounded-3xl border border-slate-100 shadow-premium">
        <div className="flex flex-col items-center">
          ...
          <p className="text-xs text-slate-400 font-semibold mt-1">Join ParkEase to start reserving parking spots.</p>
        </div>
        ...
        <div className="flex space-x-2 bg-slate-50 p-1.5 rounded-2xl border border-slate-100">
          <button
            ...
            className={`w-1/2 py-2.5 text-xs font-extrabold rounded-xl transition ${
              role === 'DRIVER' ? 'bg-white text-indigo-600 shadow-sm border border-slate-100' : 'text-slate-400 hover:text-slate-600'
            }`}
          >
            I am a Driver
          </button>
          <button
            ...
            className={`w-1/2 py-2.5 text-xs font-extrabold rounded-xl transition ${
              role === 'OWNER' ? 'bg-white text-indigo-600 shadow-sm border border-slate-100' : 'text-slate-400 hover:text-slate-600'
            }`}
          >
            I am a Lot Owner
          </button>
